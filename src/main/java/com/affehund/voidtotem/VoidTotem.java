...
public class VoidTotem {
	...
	private void enqueueIMC(final InterModEnqueueEvent event) {
		if (ModUtils.isModLoaded(ModConstants.CURIOS_MOD_ID)) {
			InterModComms.sendTo(ModConstants.CURIOS_MOD_ID,  CuriosAPI.IMC.REGISTER_TYPE,
					() -> new CurioIMCMessage("charm").setSize(1).setEnabled(true));
	...
	public void tooltip(final ItemTooltipEvent event) {
		...
		if (stack.getItem() == VoidTotem.VOID_TOTEM_ITEM.get()
			...
			event.getToolTip()
					.add(new TranslationTextComponent(ModConstants.TOOLTIP_VOID_TOTEM).applyTextStyle(TextFormatting.GREEN));
		}
	}
	...
	private void playerTick(final PlayerTickEvent event) {
		if (event.side == LogicalSide.SERVER && event.phase == TickEvent.Phase.START) {
			if (event.player instanceof ServerPlayerEntity) {
				...
				if (player.world.getBlockState(pos.offset(Direction.DOWN)).isSolid()) {
	...
	private void livingHurt(final LivingHurtEvent event) {
		...
		if (VoidTotemConfig.COMMON_CONFIG.BLACKLISTED_DIMENSIONS.get()
				.contains(event.getEntityLiving().world.getDimension().toString())) // dim on blacklist
			return;
		// no valid damage
		...
		// when totem equiped)
		if (event.getEntityLiving().getPosition().getY() > -64)
			return;
	...
	public void playActivateAnimation(ItemStack stack, Entity entity) {
		...
		mc.world.playSound(entity.getPosition().getX(), entity.getPosition().getY(), entity.getPosition().getZ(), SoundEvents.ITEM_TOTEM_USE,
